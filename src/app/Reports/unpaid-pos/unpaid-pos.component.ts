...
@Component({
  ...
  styleUrl: './unpaid-pos.component.css',
})
export class UnpaidPOSComponent {
...
displayedColumns: string[] = [
  'sno', 'po_no', 'podate', 'POQTY', 
  'facility_aut_name', 'Supplier', 'item_code_as_per_tender', 

  'itemname', 'POValue', 'Liability', 'potype', 'SupplierDispatch',
  'receiptQTY', 'fitunfit', 'EQtype', 'ReasonName', 'cancellationdays',
];
  constructor(
...
//https://localhost:7036/api/GMFI/GetUnpaidPoGridData?financialYearId=15&fromDate=2025-03-15&toDate=2026-06-15


Getitemwisedetail() {
  if (!this.selecteditem || this.selecteditem <= 0) {
    this.toastr.warning('Please select a valid Financial Year from selection panel dropdown.');
    return;
  }

  const finalFromDate = this.fromdate ? this.fromdate : '';
  const finalToDate = this.todate ? this.todate : '';

  this.spinner.show();
  
  const queryApiUrl = `GMFI/GetUnpaidPoGridData?financialYearId=${this.selecteditem}&fromDate=${finalFromDate}&toDate=${finalToDate}`;
  
  console.log("Request URL Stream Trace:", queryApiUrl);

  try {
    this.api.get(queryApiUrl).subscribe({
      next: (res: any) => {
        ...
        console.log('Data aggregated successfully:', this.dispatchData);
      ...
      error: (error: any) => {
        ...
        this.toastr.error(error.error?.message || 'Failed to extract unpaid records stream parameters data grids.');
      }
    });
  } catch (err: any) {
    this.spinner.hide();
    console.error("Runtime exception block thrown:", err);
  }
}
