...
@Component({
  selector: 'app-equipment-tag-report',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './equipment-tag-report.component.html',
  styleUrls: [
    '../../component/Suppliers/supplier-po-pages.shared.css',
    './equipment-tag-report.component.css'
  ],
})
export class EquipmentTagReportComponent {
  fromDate: string = '';
  toDate: string = '';
  poType: string = 'All';
  dispatchData: any[] = [];
  ...
  loadData() {
    this.loading = true;
    const apiUrl = `Reports/equipment-tag-report?poType=${this.poType}&fromDate=${this.fromDate}&toDate=${this.toDate}`;
    ...
    this.api.get(apiUrl).subscribe({
      ...
      error: (err: any) => {
        this.loading = false;
        if (err.status === 404) {
          this.toastr.warning('Equipment Tag Report API not available yet. Showing placeholder.');
          this.dispatchData = [];
