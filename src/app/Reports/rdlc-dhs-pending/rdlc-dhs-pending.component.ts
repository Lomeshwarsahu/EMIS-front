...
@Component({
  ...
  templateUrl: './rdlc-dhs-pending.component.html',
  styleUrls: [
    '../../component/Suppliers/supplier-po-pages.shared.css',
    './rdlc-dhs-pending.component.css'
  ],
})
export class RdlcDhsPendingComponent {
  ...
  displayedColumns: string[] = [
    'sno', 'po_no', 'po_date', 'item_code', 'item_name',
    'po_qty', 'receipt_qty', 'install_qty', 'pending_receipt',
    'pending_install', 'supplier_name', 'directorate_name'
  ];
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      this.directorateId = params['directorateId'] ? Number(params['directorateId']) : 0;
  ...
  loadData() {
    this.loading = true;
    this.api.get(`Reports/rdlc-dhs-pending?directorateId=${this.directorateId}`).subscribe({
      next: (res: any) => {
