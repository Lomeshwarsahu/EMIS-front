...
@Component({
  selector: 'app-balance-status-supplier',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './balance-status-supplier.component.html',
  styleUrls: [
    '../../component/Suppliers/supplier-po-pages.shared.css',
    './balance-status-supplier.component.css'
  ],
})
export class BalanceStatusSupplierComponent {
  balanceType: string = 'R';
  ...
  displayedColumns: string[] = [
    'sno', 'tender_no', 'year', 'po_no', 'po_date', 'authority',
    'item_code', 'item_name', 'supplier', 'po_qty', 'supply_qty',
    'receipt_qty', 'install_qty', 'po_type', 'balance_qty',
  ];
  ...
  loadData() {
    this.loading = true;
    this.api.get(`Reports/balance-status-supplier?balanceType=${this.balanceType}`).subscribe({
      next: (res: any) => {
  ...
  exportToExcel() {
    ...
    if (exporter && exporter.exportTable) {
      exporter.exportTable('xlsx', {
        fileName: 'BalanceStatusSupplier',
        sheet: 'Data',
        Props: { Author: 'cgmsc' },
      });
    }
