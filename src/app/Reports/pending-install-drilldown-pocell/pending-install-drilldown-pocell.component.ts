...
@Component({
  selector: 'app-pending-install-drilldown-pocell',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './pending-install-drilldown-pocell.component.html',
  styleUrls: [
    '../../component/Suppliers/supplier-po-pages.shared.css',
    './pending-install-drilldown-pocell.component.css'
  ],
})
export class PendingInstallDrilldownPocellComponent {
  poId: number = 0;
  dispatchData: any[] = [];
  ...
  loadData() {
    this.loading = true;
    this.api.get(`Reports/pending-install-drilldown-pocell?POID=${this.poId}`).subscribe({
      next: (res: any) => {
  ...
  exportToExcel() {
    ...
    if (exporter && exporter.exportTable) {
      exporter.exportTable('xlsx', {
        fileName: 'PendingInstallDrilldownPOCell',
        sheet: 'Data',
        Props: { Author: 'cgmsc' },
      });
    }
