...
@Component({
  ...
  templateUrl: './po-summary-drilldown-qty.component.html',
  styleUrls: [
    '../../component/Suppliers/supplier-po-pages.shared.css',
    './po-summary-drilldown-qty.component.css',
  ],
})
export class PoSummaryDrilldownQtyComponent {
  finYrId: string = '';
  itemCode: string = '';
  directorateId: string = '';
  flag: string = '';

  detailData: POSummaryDetailDTO[] = [];
  dataSource = new MatTableDataSource<POSummaryDetailDTO>([]);
  ...
  loadData() {
    ...
    this.api
      .get(
        `Reports/po-summary-detail?finYrId=${this.finYrId}&itemCode=${this.itemCode}&directorateId=${this.directorateId}`,
      )
      .subscribe({
        next: (res: any) => {
  ...
  exportToExcel() {
    ...
    if (exporter && exporter.exportTable) {
      exporter.exportTable('xlsx', {
        fileName: 'POSummaryDrilldownQty',
        sheet: 'Data',
        Props: { Author: 'cgmsc' },
      });
    }
