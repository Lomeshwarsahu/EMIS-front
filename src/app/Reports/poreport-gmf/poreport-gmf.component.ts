...
@Component({
  ...
  templateUrl: './poreport-gmf.component.html',
  styleUrl: './poreport-gmf.component.css',
})
export class POReportGMFComponent {
...
displayedColumns: string[] = [
  'sno', 'facility_aut_code', 'PO_NO', 'po_date', 'tender_no', 
  'CODE', 'ITEM_NAME', 'basic_rate', 'quantity', 'totalPOvalue', 'status','print'
];
  constructor(
...
// Core dynamic trigger operation pipeline
fetchPurchaseOrderGrid() {
  this.spinner.show();
  
  // Enforcing type convert layer safety checklist
  const yearId = this.financial_year_id ;
  // const yearId = this.selectedFinancialYearId ? this.financial_year_id : 0;
  
  // Query strings route pattern parsing sequence
  const queryUrl = `GMFI/GetPurchaseOrderGridData?financialYearId=${yearId}`;

  this.api.get(queryUrl).subscribe({
    next: (res: any) => {
      this.purchaseOrderData = (res || []).map((item: any, index: number) => ({
        ...item,
        sno: index + 1
      }));
    ...
    error: (err) => {
      this.spinner.hide();
      console.error(err);
      this.toastr.error(err.error?.message || 'Failed to aggregate active purchase order ledger summaries.');
    }
  });
}
