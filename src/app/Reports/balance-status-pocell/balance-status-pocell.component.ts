...
@Component({
  selector: 'app-balance-status-pocell',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './balance-status-pocell.component.html',
  styleUrls: [
    '../../component/Suppliers/supplier-po-pages.shared.css',
    './balance-status-pocell.component.css'
  ],
})
export class BalanceStatusPocellComponent {
  ...
  loadData() {
    this.loading = true;
    this.api.get(`Reports/balance-status?balanceType=${this.balanceType}`).subscribe({
      next: (res: any) => {
