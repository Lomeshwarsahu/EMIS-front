...
@Component({
  ...
  styleUrls: [
    ...
    './item-wise-detail-pocell.component.css'
  ],
})
export class ItemWiseDetailPOCellComponent {
  ...
  loadData() {
    ...
    this.loading = true;
    this.api.get(`Reports/itemwisedetail-pocell?itemId=${this.selectedItem}`).subscribe({
      next: (res: any) => {
  ...
  navigateToPOQty(element: ItemWiseDetailDTO) {
    this.router.navigate(['/reports/item-wise-detail-poqty-pocell'], {
      queryParams: {
