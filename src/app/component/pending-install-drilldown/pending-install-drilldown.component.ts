...
@Component({
  selector: 'app-pending-install-drilldown',
  standalone: true,
...
<div class="it-page" style="--page-accent:#dc2626">
  <h2 class="page-title">Pending Installation Drill Down</h2>
  <div class="datagrid">
    <div class="filters--compact">
      ...
      <button type="button" class="show-btn" (click)="loadGrid()">Show</button>
      <div class="info-badges" *ngIf="totalCount > 0">
        <span class="badge">Total: {{ totalCount }} rows</span>
      </div>
    </div>
    <app-dme-page-skeleton *ngIf="loading" type="table" [rows]="6" [cols]="10" />
    <div class="table-wrap" *ngIf="!loading">
      <table mat-table [dataSource]="dataSource" matSort #sort="matSort" class="it-grid">
        ...
        <ng-container matColumnDef="district"><th mat-header-cell *matHeaderCellDef>District</th><td mat-cell *matCellDef="let row">{{ row.districtName }}</td></ng-container>
        ...
        <ng-container matColumnDef="poNo"><th mat-header-cell *matHeaderCellDef>PO No</th><td mat-cell *matCellDef="let row">{{ row.poNo }}</td></ng-container>
        <ng-container matColumnDef="poDate"><th mat-header-cell *matHeaderCellDef>PO Date</th><td mat-cell *matCellDef="let row">{{ row.poDate }}</td></ng-container>
        <ng-container matColumnDef="qty"><th mat-header-cell *matHeaderCellDef>Qty</th><td mat-cell *matCellDef="let row">{{ row.quantity }}</td></ng-container>
        <ng-container matColumnDef="dispatched"><th mat-header-cell *matHeaderCellDef>Dispatched</th><td mat-cell *matCellDef="let row">{{ row.dispatchedQty }}</td></ng-container>
        <ng-container matColumnDef="received"><th mat-header-cell *matHeaderCellDef>Received</th><td mat-cell *matCellDef="let row">{{ row.receiptQty }}</td></ng-container>
        ...
        <ng-container matColumnDef="remarks"><th mat-header-cell *matHeaderCellDef>Remarks</th><td mat-cell *matCellDef="let row">{{ row.remarks }}</td></ng-container>
        <tr mat-header-row *matHeaderRowDef="displayedColumns; sticky: true"></tr>
        <tr mat-row *matRowDef="let row; columns: displayedColumns;"></tr>
      </table>
      <div class="empty-row" *ngIf="!dataSource.data.length">Enter PO ID and click Show.</div>
      <mat-paginator #paginator [pageSizeOptions]="[10,15,30,50]" showFirstLastButtons></mat-paginator>
    </div>
  </div>
</div>`,
...
.info-badges{display:flex;gap:10px;margin-left:12px}
.badge{background:#e2e8f0;padding:2px 12px;border-radius:999px;font-size:12px;font-weight:600}
:host-context(html[data-theme='dark']) .badge{background:#262b36;color:#e2e8f0}
...
export class PendingInstallDrilldownComponent {
  poId = '';
  totalCount = 0;
  loading = false;
  dataSource = new MatTableDataSource<any>([]);
  displayedColumns = ['sno', 'district', 'location', 'itemCode', 'itemName', 'supplier', 'poNo', 'poDate', 'qty', 'dispatched', 'received', 'installed', 'remarks'];
  @ViewChild('sort') sort!: MatSort;
  @ViewChild('paginator') paginator!: MatPaginator;

  constructor(private readonly api: ApiService, private readonly toastr: ToastrService, route: ActivatedRoute) {
    route.queryParams.subscribe(p => {
      if (p['poid'] || p['POID']) { this.poId = p['poid'] || p['POID']; setTimeout(() => this.loadGrid()); }
    });
  }
  ...
  loadGrid() {
    if (!this.poId) return;
    this.loading = true;
    this.api.get('PendingInstallDrillDown/grid', { params: { poId: this.poId } }).subscribe({
      next: (res: any) => {
