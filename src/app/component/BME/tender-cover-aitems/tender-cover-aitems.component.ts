...
import {GetTenderItemsDTO, TenderSupplierParticipationDto } from 'src/app/Model/models';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-tender-cover-aitems',
    standalone: true,
  imports: [
    NgSelectModule,
    CommonModule,
  ...
  templateUrl: './tender-cover-aitems.component.html',
  styleUrl: './tender-cover-aitems.component.css',
})
export class TenderCoverAitemsComponent {
tenderNo: string | null = null;
 ...
 item_id:any;
 Sname:any;
  dispatchData: GetTenderItemsDTO[] = [];
   dataSource!: MatTableDataSource<GetTenderItemsDTO>;
...
//   'checkbid'
// ];
displayedColumns: string[] = ['sno', 'ItemCodeAsPerTender','ItemName','select'];
  SchStatusDid: any;
  Supplierid: any;
 constructor(
   ...
   this.dataSource = new MatTableDataSource<GetTenderItemsDTO>([]);
  }
    ngOnInit(): void {
...
GetLinkedItems() {
  ...
  // https://localhost:7036/api/BME/GetTenderItems/680
    try {
      this.spinner.show();
      this.api.get(`BME/GetTenderItems/${this.tenderNo}`).subscribe(
        (res: any) => {
          this.dispatchData = res.map((item: GetTenderItemsDTO, index: number) => ({
            ...item,
            sno: index + 1,
          }));
