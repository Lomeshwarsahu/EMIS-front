...
import { ContractGridDetailDTO, TenderSupplierDataDTO } from 'src/app/Model/models';

@Component({
  selector: 'app-rcextend',
   standalone: true,
  imports: [
    NgSelectModule,
    CommonModule,
  ...
  templateUrl: './rcextend.component.html',
  styleUrl: './rcextend.component.css',
})
export class RCExtendComponent {
  Tenterlist: any[] = [];
  Supplierlist: any[] = [];
  txtRmrk: any;
  selectedvalue:any;
...
isSupplierDisabled: boolean = false;
  dispatchData: ContractGridDetailDTO[] = [];
  dataSource!: MatTableDataSource<ContractGridDetailDTO>;
  ...
  constructor(
    ...
    this.dataSource = new MatTableDataSource<ContractGridDetailDTO>([]);
  }
...
// https://localhost:7036/api/BME/GetTenderList1/5
  ...
  //https://localhost:7036/api/POCell/GetContractGridDetail?tenderId=169&awardOfContractId=382
  GetTenderSupplierData() {
    ...
    try {
      ...
      this.api.get(`POCell/GetContractGridDetail?tenderId=${ this.Tenderid}&awardOfContractId=${this.AwardOfContractId}`).subscribe(
        (res: any) => {
          this.dispatchData = res.map((item: ContractGridDetailDTO, index: number) => ({
            ...item,
            sno: index + 1,
          }));
