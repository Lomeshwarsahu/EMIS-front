...
@Component({
  selector: 'app-consolidated-indent-cgmsc',
     standalone: true,
  imports: [
    NgSelectModule,
    CommonModule,
  ...
  templateUrl: './consolidated-indent-cgmsc.component.html',
  styleUrl: './consolidated-indent-cgmsc.component.css',
})
export class ConsolidatedIndentCGMSCComponent {
Yearlist: any = {};
Diectoratelist: any = {};
dmeInstitueList: any = {};
selecteditem=20;
...
GetDiectorate() {
  ...
  this.api.get(`Reports/GetDiectorate`).subscribe({
    next: (res: any) => {
      this.Diectoratelist = res.filter((item: any) => 
        item.facility_aut_id === 11 || item.facility_aut_id === 12
      ); 

      console.log('Filtered Directorates:', this.Diectoratelist);
      this.spinner.hide();
    },
    error: (err: any) => {
      console.error(err);
...
// https://localhost:7036/api/Tendercell/GetDmeInstitutes/11
onDirectorateChange(directorateId: number) {
  // debugger
  if (directorateId === 0) {
    this.dmeInstitueList = [{ UserId: 0, UserName: 'All' }];
    return;
  }

  this.api.get(`Tendercell/GetDmeInstitutes/${directorateId}`).subscribe({
    next: (res: any) => {
      this.dmeInstitueList = [{ UserId: 0, UserName: 'All' }, ...res];
    },
    error: (err) => {
...
onSelectedDiectorate(Diectorate: any) {
  ...
  this.onDirectorateChange(this.Diectorateid);
