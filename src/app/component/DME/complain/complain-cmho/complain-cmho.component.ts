...
@Component({
  selector: 'app-complain-cmho',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './complain-cmho.component.html',
  styleUrls: ['./complain-cmho.component.css'],
})
export class ComplainCmhoComponent implements OnInit {
  ...
  load(): void {
    ...
    this.http
      .get<ComplainStatusRow[]>(
        `${this.apiRoot}cmho-status-list?userId=${this.userId}&status=${encodeURIComponent(this.status)}`,
      )
      .subscribe({
        next: (res) => {
